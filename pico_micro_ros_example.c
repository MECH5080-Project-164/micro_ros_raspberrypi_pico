...
const uint8_t VOLTAGE_SAMPLE_COUNT = 32; // Number of ADC samples to average for voltage reading
...
float read_battery_voltage(void) {
    // Ensure we're reading from the correct ADC input
    adc_select_input(0); // ADC0 for GPIO26 battery monitoring
...
void init_single_pwm(pwm_config_t* config) {
    ...
    // Debug: log which slice and channel are used for this pin
    char slice_log[64];
...
void handle_pwm_message(pwm_channel_e channel, const std_msgs__msg__Int32* msg_in) {
    ...
    uint32_t pulse_us = min_p + ((uint32_t)servo_angle * (max_p - min_p)) / 180;
        ...
        snprintf(log_msg_local, sizeof(log_msg_local), "Servo%d: Set to %d° (%dμs pulse, level=%d)",
                 pwm_configs[channel].pin, servo_angle, pulse_us, pwm_level);
    } else {
